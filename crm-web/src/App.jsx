...
import {
  ...
  fromApiTask,
  fromApiUser,
  membershipStatusForContact,
  subscriberStatusCode,
  toApiContactPayload,
  toApiMembershipPayload,
...
import {
  normalizeMembershipPricingCatalog,
  normalizeMembershipPricingQuote,
...
const startupConfigurationError =
  authClient.configurationError || apiConfigurationError;
...
export function salesForDashboard(sales, { executiveName, from, to } = {}) {
  ...
  return sales.filter((sale) => {
    ...
    if ((fromTime !== null || toTime !== null) && !Number.isFinite(occurredAt))
      return false;
...
export function buildSaleItems({
  ...
  const discountSuffix = discountCode
    ? ` · DESCUENTO ${discountName || discountCode} [${discountCode}]`
    : "";
  if (pricingMode === "two_for_one") {
    return [
      {
        product: `${product}${discountSuffix} · PROMOCIÓN 2X1 (CON CARGO)`,
        zone,
        quantity: chargedUnits,
        unitPrice,
      },
      ...(bonusUnits
        ? [
            {
              product: `${product}${discountSuffix} · PROMOCIÓN 2X1 (BONIFICADO)`,
              zone,
              quantity: bonusUnits,
              unitPrice: 0,
            },
  ...
  if (!promotion2x1)
    return [
      {
        product: `${product}${discountSuffix}`,
        zone: zone || undefined,
        quantity,
        unitPrice,
      },
  ...
  return [
    {
      product: `${product} · PROMOCIÓN 2X1 (CON CARGO)`,
      zone,
      quantity: fallbackChargedUnits,
      unitPrice,
    },
    ...(fallbackBonusUnits
      ? [
          {
            product: `${product} · PROMOCIÓN 2X1 (BONIFICADO)`,
            zone,
            quantity: fallbackBonusUnits,
            unitPrice: 0,
          },
        ]
      : []),
...
export function buildCommitmentItems({
  suiteNumber,
  quantity,
  totalAmount,
  coverageSeasons = 2,
  ...
  if (
    ...
    return [];
  const baseCents = Math.floor(totalCents / seats);
  const higherPriceSeats = totalCents % seats;
  const product = `COMPROMISO ANUAL DE SUITE · ${coverageSeasons} TEMPORADAS`;
  const zone = `Suite ${String(suiteNumber || "").trim()}`;
  return [
    ...(higherPriceSeats
      ? [
          {
            product,
            zone,
            quantity: higherPriceSeats,
            unitPrice: (baseCents + 1) / 100,
          },
        ]
      : []),
    ...(seats - higherPriceSeats
      ? [
          {
            product,
            zone,
            quantity: seats - higherPriceSeats,
...
export function saleItemFacet(item = {}) {
  const text = `${item.product || ""} ${item.zone || ""}`.toLowerCase();
  if (text.includes("estacionamiento")) return "Estacionamientos";
  if (
    text.includes("compromiso") ||
    text.includes("zona suites") ||
    /^suite\s/.test(text)
  )
    return "Compromisos";
  if (text.includes("vip") || text.includes("suites")) return "VIP";
  if (
    text.includes("preferente") ||
...
export function saleAmountForFacets(sale, facets = []) {
  if (!facets.length) return Number(sale.total || sale.totalAmount || 0);
  return (sale.items || [])
    .filter((item) => facets.includes(saleItemFacet(item)))
    .reduce(
      (sum, item) =>
...
function StatusPill({ children, tone }) {
  ...
  return (
    <span className={`status-pill status-pill--${inferred}`}>
...
function getTone(value) {
  ...
  if (/(pagado|abonado actual|respondió|completada|finalizada|sí)/.test(text))
    return "success";
  if (/(vencida|eliminado|no interesado|incorrecto|no consta)/.test(text))
    return "danger";
  if (/(interesado|apartado|parcial|seguimiento|en curso)/.test(text))
    return "gold";
  ...
  return (
    ...
    entries.every(
      ([field, expected]) =>
        ...
        normalizeContactPatchValue(field, contact[field]) ===
          normalizeContactPatchValue(field, expected),
    )
  );
}
...
export async function updateContactWithVerification(
  api,
  id,
  patch,
  rowVersion,
  ...
  try {
    ...
    if (!response?.data || typeof response.data !== "object")
      throw new Error("El servidor no devolvió el contacto actualizado.");
  ...
  if (verified) {
    try {
      return fromApiContact(verified);
    } catch {
      /* La confirmación también debe ser hidratable. */
    }
  }
  throw originalError;
}
...
function App() {
  ...
  const [membershipPricingCatalog, setMembershipPricingCatalog] =
    useState(null);
  const [bootState, setBootState] = useState(
    startupConfigurationError ? "error" : "loading",
  );
  ...
  useEffect(
    () =>
      authClient.subscribe(({ reason, session }) => {
  ...
  useEffect(() => {
    if (authClient.isDemo || bootState !== "ready" || !sessionDeadline)
      return undefined;
  ...
  useEffect(() => {
    ...
    async function loadDemo() {
      try {
        if (!loadDemoModule)
          throw new Error("Los datos demo no están disponibles en producción.");
        const fixtures = await loadDemoModule();
        if (!active) return;
        setUser(fixtures.demoUser);
        setContacts(fixtures.demoContacts);
        const demoUnassigned = fixtures.demoContacts.filter(
          (contact) => contact.executive === "Sin asignar",
        );
        ...
        setMembershipPricingCatalog(
          normalizeMembershipPricingCatalog(
            fixtures.demoMembershipPricingCatalog,
          ),
        );
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        const mayUseExecutiveDirectory =
          hasPermission(currentUser, PERMISSIONS.CONTACT_ASSIGN) ||
        ...
        const todayOpenData = Array.isArray(todayOpenResponse.data)
          ? todayOpenResponse.data
          : todayOpenResponse.data?.items || [];
        const todayCompletedData = Array.isArray(todayCompletedResponse.data)
          ? todayCompletedResponse.data
          : todayCompletedResponse.data?.items || [];
        const overdueTaskData = Array.isArray(overdueResponse.data)
          ? overdueResponse.data
          : overdueResponse.data?.items || [];
        const salesData = salesResponse.data;
        const executiveData = executiveResponse.data;
        setUser(currentUser);
        setDashboardSummary(summaryData);
        setMembershipPricingCatalog(
          normalizeMembershipPricingCatalog(pricingCatalogResponse),
        );
        const combinedTasks = [
          ...allOpenData,
          ...todayCompletedData,
        ...
        setTasks(
          [
            ...new Map(combinedTasks.map((item) => [item.id, item])).values(),
          ].map(fromApiTask),
        );
        setFollowupCounts({
          scheduled:
            Number(todayOpenResponse.meta?.total || 0) +
            Number(todayCompletedResponse.meta?.total || 0),
        ...
        setSales(
          (Array.isArray(salesData) ? salesData : salesData?.items || []).map(
            fromApiSale,
          ),
        );
        setAvailableExecutives(
        ...
        const unassignedData = Array.isArray(unassignedResponse.data)
          ? unassignedResponse.data
          : unassignedResponse.data?.items || [];
        setUnassignedContacts(unassignedData.map(fromApiContact));
        setUnassignedTotal(Number(unassignedResponse.meta?.total || 0));
        const interactionData = Array.isArray(interactionResponse.data)
          ? interactionResponse.data
          : interactionResponse.data?.items || [];
        ...
        if (error.status === 403) {
          setBootError(
            "Tu sesión es válida, pero no cuenta con permisos para abrir este módulo.",
          );
          setBootState("error");
          return;
        }
  ...
  async function logout() {
    ...
    try {
      await revokeSessionSafely(api, (reason) =>
        authClient.clearSession(reason),
      );
    } catch {
      setUserOpen(false);
      setToast(
        "No fue posible cerrar la sesión. Tu acceso continúa activo; inténtalo nuevamente.",
      );
    }
  }
  ...
  async function saveContact(payload) {
    try {
      ...
      if (payload.id) {
        ...
        if (hasPermission(user, PERMISSIONS.MEMBERSHIP_WRITE))
          allowedFields.add("subscriberStatus");
        if (hasPermission(user, PERMISSIONS.CONTACT_ASSIGN))
          allowedFields.add("executiveId");
        if (hasPermission(user, PERMISSIONS.CONTACT_WRITE_ALL))
          allowedFields.add("consentStatus");
      ...
      if (authClient.isDemo) {
        if (payload.id)
          ...
          setContacts((current) => [
            fromApiContact({
              ...apiPayload,
              id,
              displayName: `${apiPayload.firstName} ${apiPayload.lastName}`,
              seatCount: 0,
              seasonsCount: 0,
              rowVersion: 1,
            }),
        ...
        const contact = await updateContactWithVerification(
          api,
          payload.id,
          apiPayload,
          payload.rowVersion,
        ...
        setContacts((current) =>
          current.map((item) => (item.id === payload.id ? contact : item)),
        );
      } else {
  ...
  async function saveManualRegistration(draft, idempotencyKey) {
    ...
    if (authClient.isDemo) {
      ...
      const executiveName =
        availableExecutives.find(
          (item) => item.id === payload.contact.executiveId,
        )?.displayName ||
      ...
      const contact = {
        ...
        seatCount: ["current_subscriber", "new_subscriber"].includes(
          payload.contact.subscriberStatus,
        )
          ? membershipSeats
          : 0,
      ...
      const nextTask = payload.nextTask
        ? {
            ...
            assigneeName:
              availableExecutives.find(
                (item) => item.id === payload.nextTask.assignedTo,
      ...
      setInteractions((current) => [
        fromApiInteraction(initialInteraction),
        ...current,
      ]);
      setDashboardSummary((current) =>
        current
          ? {
              ...
              renewing:
                Number(current.renewing || 0) +
                (contact.subscriberStatus === "renewing" ? 1 : 0),
              newSubscribers:
                Number(current.newSubscribers || 0) +
                (contact.subscriberStatus === "new_subscriber" ? 1 : 0),
              currentSubscribers:
                Number(current.currentSubscribers || 0) +
                (contact.subscriberStatus === "current_subscriber" ? 1 : 0),
              activeSeats:
                Number(current.activeSeats || 0) +
                Number(contact.seatCount || 0),
              unassigned:
                Number(current.unassigned || 0) +
                (!contact.executiveId ? 1 : 0),
            }
          : current,
      );
    } else {
      const response = await api.createManualRegistration(
        payload,
        idempotencyKey,
      );
      ...
      if (result?.contact) {
        ...
        setContacts((current) => [
          normalizedContact,
          ...current.filter((item) => item.id !== normalizedContact.id),
        ]);
      }
      if (result?.initialInteraction) {
        const normalized = fromApiInteraction({
          ...
          contactName:
            result.initialInteraction.contactName ||
      ...
      try {
        const unassignedResponse = await api.contacts({
    ...
    if (result?.nextTask) {
      const normalized = fromApiTask({
        ...result.nextTask,
        contactName:
          result.nextTask.contactName ||
          result.contact?.displayName ||
          `${result.contact?.firstName || ""} ${result.contact?.lastName || ""}`.trim(),
        assigneeName:
          result.nextTask.assigneeName ||
          availableExecutives.find(
            (item) => item.id === result.nextTask.assignedTo,
          )?.displayName ||
          user.name,
      });
      const today = localDayBounds();
      ...
      const dueToday =
        dueTime >= new Date(today.from).getTime() &&
        dueTime <= new Date(today.to).getTime();
      const overdue = dueTime < Date.now();
      setTasks((current) => [
        normalized,
        ...current.filter((item) => item.id !== normalized.id),
      ]);
      setFollowupCounts((current) =>
        current
          ? {
              ...current,
    ...
    if (resumeSale) {
      setSaleClosure({
        contact: fromApiContact(result.contact),
        stage: "Apartado",
      });
      navigate("sales");
    }
    setToast(
      resumeSale
        ? "Contacto creado. Completa ahora el número de orden y los datos de la venta."
        : result?.replayed
          ? "El alta ya se había procesado; la cartera se actualizó sin duplicar."
          : "El registro se creó correctamente y la cartera fue actualizada.",
    );
    return result;
  }
  ...
  async function correctSale(sale, payload) {
    ...
    setDashboardRevision((current) => current + 1);
    setToast(
      "La corrección quedó registrada y los indicadores fueron actualizados.",
    );
  }
  ...
  async function cancelSale(sale, reason) {
    ...
    setDashboardRevision((current) => current + 1);
    setToast(
      `La orden ${sale.externalOrderNumber || sale.id} quedó anulada y los indicadores fueron actualizados.`,
    );
  }
  ...
  async function openContact(contactOrId, options = {}) {
    ...
    const includeDeleted =
      ...
      Boolean(typeof contactOrId === "object" && contactOrId?.deletedAt);
    ...
    try {
      ...
      if (authClient.isDemo) {
        ...
        memberships =
          detailedContact?.memberships ||
          (detailedContact?.currentMembership
            ? [detailedContact.currentMembership]
            : []);
      } else {
        ...
        memberships = Array.isArray(membershipsResponse.data)
          ? membershipsResponse.data
          : membershipsResponse.data?.items || [];
      }
      if (!detailedContact)
        throw new Error("No encontramos el contacto coincidente.");
      const normalizedMemberships = memberships
        .map(fromApiMembership)
        .filter(Boolean);
      const membership = currentSeasonMembership(normalizedMemberships);
      const contact = {
        ...fromApiContact(detailedContact),
  ...
  async function saveMembership(contact, membership, draft) {
    ...
    if (!membershipStatus)
      throw new Error(
        "Los prospectos no pueden recibir abonos hasta cambiar su clasificación.",
      );
    ...
    try {
      ...
      if (authClient.isDemo) {
        const fixtures = await loadDemoModule();
        const pricing = normalizeMembershipPricingQuote(
          fixtures.quoteDemoMembershipPricing(payload),
        );
        const saved = fromApiMembership({
          ...
          membershipStatus,
          seasonCode:
            payload.seasonCode || membership?.seasonCode || "LMP-2026-27",
        ...
        refreshedMemberships = [
          saved,
          ...(contact.memberships || []).filter((item) => item.id !== saved.id),
        ];
        refreshedContact = {
          ...contact,
          seats: saved.seatCount,
          currentMembership: saved,
        };
      } else {
        if (membership)
        ...
        refreshedMemberships = (
          ...
          .filter(Boolean);
        refreshedContact = {
          ...fromApiContact(contactResponse.data),
          currentMembership: currentSeasonMembership(refreshedMemberships),
        };
      }
      const contactCurrentMembership =
        refreshedContact.currentMembership ||
        currentSeasonMembership(refreshedMemberships);
      const refreshedMembership = membership
        ? refreshedMemberships.find((item) => item.id === membership.id) ||
          membership
        ...
        current.map((item) =>
          item.id === contact.id
            ? {
                ...item,
                ...refreshedContact,
                currentMembership: contactCurrentMembership,
              }
            : item,
        ),
      );
      ...
      setToast(
        membership
          ? "Los abonos y butacas se actualizaron correctamente."
          : "Los abonos y butacas se agregaron correctamente.",
      );
  ...
  async function createInteraction(contact, payload) {
    try {
      ...
      if (authClient.isDemo) {
        ...
        created = {
          ...response.data,
          contactName: response.data?.contactName || contact.name,
          actorName: response.data?.actorName || user.name,
        };
      }
  ...
  async function createTask(contact, payload) {
    try {
      let created;
      const assigneeName =
        availableExecutives.find((item) => item.id === payload.assignedTo)
          ?.displayName ||
        (payload.assignedTo === user.id ? user.name : "Responsable asignado");
      if (authClient.isDemo) {
        const { nextDemoRecordId } = await loadDemoModule();
        created = {
          ...payload,
        ...
        created = {
          ...response.data,
          contactName: response.data?.contactName || contact.name,
          assigneeName: response.data?.assigneeName || assigneeName,
        };
      }
      ...
      setTasks((current) => [
        normalizedTask,
        ...current.filter((item) => item.id !== normalizedTask.id),
      ]);
      setFollowupCounts((current) =>
        current
          ? {
    ...
    try {
      ...
      if (authClient.isDemo)
        updated = {
          ...task,
          status: "Completada",
          rowVersion: Number(task.rowVersion || 0) + 1,
        };
      else {
        ...
        updated = fromApiTask({
          ...response.data,
          contactName: response.data?.contactName || task.contact,
          assigneeName: response.data?.assigneeName || task.owner,
        });
      }
      setTasks((current) =>
      ...
      setFollowupCounts((current) =>
        current
          ? {
              ...
              overdue: Math.max(
                0,
                current.overdue - (task.status === "Vencida" ? 1 : 0),
              ),
  ...
  async function softDeleteContact(contact, reason) {
    try {
      ...
      setUnassignedContacts((current) =>
        current.filter((item) => item.id !== contact.id),
      );
      if (!contact.executiveId)
        setUnassignedTotal((current) => Math.max(0, current - 1));
      setFollowupCounts(null);
      setContactRevision((current) => current + 1);
      setDashboardRevision((current) => current + 1);
      setDrawer(null);
  ...
  async function restoreContact(contact) {
    try {
      if (!authClient.isDemo) {
        const { data } = await api.restoreContact(
          contact.id,
          contact.rowVersion,
        );
        setContacts((current) =>
          current.map((item) =>
            item.id === contact.id ? fromApiContact(data) : item,
          ),
        );
      } else {
        setContacts((current) =>
          current.map((item) =>
            item.id === contact.id ? { ...item, deletedAt: null } : item,
          ),
        );
      }
      setContactRevision((current) => current + 1);
  ...
  async function exportSubscriberDetail(filters = {}) {
    if (authClient.isDemo)
      throw new Error("Modo demostracion: no se descargan datos reales.");
        ...
        setContacts((current) =>
          normalized.map((contact) => {
            ...
            return !Object.prototype.hasOwnProperty.call(
              ...
              ? { ...contact, currentMembership: cached }
              : contact;
          }),
        );
      }
  ...
  const quoteMembershipPricing = useCallback(
    async (params) => {
      if (authClient.isDemo) {
        const fixtures = await loadDemoModule();
        return normalizeMembershipPricingQuote(
          fixtures.quoteDemoMembershipPricing(params),
        );
      }
      return normalizeMembershipPricingQuote(
        await api.membershipPricingQuote(params),
      );
    },
    [api],
  );
  ...
  if (bootState === "signed-out")
    return <LoginScreen onLogin={login} notice={loginNotice} />;
  if (bootState === "error")
    return (
      <ErrorScreen
        message={bootError}
  ...
  const context = {
    ...
    user,
    onEdit: openContact,
    onCreate: (kind = "portfolio", options = {}) =>
      setDrawer({ mode: "create", kind, ...options }),
    onNotify: setToast,
    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        ...
        <button
          ...
          aria-label="Ir al Reporte Dirección"
        >
          <img src="/charros-logo.jpeg" alt="Charros de Jalisco" />
          <span className="brand-copy">
            <strong>CRM Abonados</strong>
            <small>LMP 2026–2027</small>
          </span>
        </button>
      ...
      <main id="main-content" tabIndex="-1">
        ...
        {activePage === "portfolio" && (
          <ContactsPage {...context} kind="portfolio" />
        )}
        {activePage === "prospects" && (
          <ContactsPage {...context} kind="prospect" />
        )}
        {activePage === "followup" && <FollowupPage {...context} />}
        {activePage === "sales" && <SalesPage {...context} />}
        {MORE_ITEMS.some((item) => item.id === activePage) && (
          <MorePage {...context} page={activePage} />
        )}
      </main>
...
export function LoadingScreen() {
  return (
    <main className="gate-screen">
      <div className="gate-card" role="status" aria-live="polite">
        <div className="loading-brand">
          <img src="/charros-logo.jpeg" alt="Charros de Jalisco" />
        </div>
        <div className="spinner" />
        <h1>Cargando CRM…</h1>
        <p>Validando la sesión segura y actualizada.</p>
      </div>
    </main>
  );
}
  ...
  return (
    <main className="gate-screen">
      <div className="gate-card gate-card--login">
        <img
          className="gate-logo"
          src="/charros-logo.jpeg"
          alt="Charros de Jalisco"
        />
        <span className="eyebrow">Plataforma interna</span>
        <h1>CRM Abonados</h1>
        <p>
          Ingresa con las credenciales administrativas autorizadas para
          continuar.
        </p>
        {notice && (
        ...
        >
          <label className="field">
            ...
            <input
              ...
              placeholder="nombre@charrosjalisco.com"
            />
          </label>
          <label className="field">
  ...
  return (
    <main className="gate-screen">
      <div className="gate-card">
        <div className="gate-error">!</div>
        <h1>No pudimos abrir el CRM</h1>
        <p>{message}</p>
        <SecondaryButton icon="refresh" onClick={onRetry}>
          Intentar de nuevo
        </SecondaryButton>
      </div>
    </main>
    ...
    <div className="global-filters" aria-label="Filtros del reporte">
      <label>
        ...
        <select
          ...
          onChange={(event) =>
            onChange({ ...filters, season: event.target.value })
          }
        >
          <option value="LMP-2026-27">LMP 2026-2027</option>
        </select>
      </label>
      <label>
        <span>Periodo</span>
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) =>
            onChange({ ...filters, executiveId: event.target.value })
          }
        >
      ...
      {disabled && (
        <small className="demo-filter-note">
          Filtros disponibles con el API corporativo.
        </small>
      )}
    </div>
  );
...
function SegmentedMetricCard({
  ...
  return (
    <article className="metric-card metric-card--segmented">
      ...
      <div className="metric-copy">
        <span>{label}</span>
        <strong>{integer.format(active.value || 0)}</strong>
        <small>{detail}</small>
      </div>
      <div
        className="metric-segments"
        role="group"
        aria-label={`Segmentación de ${label}`}
      >
        {options.map((option) => (
          <button
            type="button"
...
function DashboardPage({
  ...
  onNotify,
  onLoadDashboard,
  onAuthorizeDashboardPdf,
  onExportSubscriberDetail,
}) {
  ...
  const selectedExecutiveName = availableExecutives.find(
    (item) => item.id === reportFilters.executiveId,
  )?.displayName;
  const demoContactsInScope =
    isDemo && selectedExecutiveName
      ? contacts.filter(
          (contact) => contact.executive === selectedExecutiveName,
        )
      : contacts;
  ...
  const demoNewSubscribers = isDemo
    ? demoContactsInScope.filter((contact) => {
        if (contact.type !== "Abonado nuevo" || !contact.renewalDate)
          return false;
        const occurredAt = new Date(contact.renewalDate).getTime();
        if (!Number.isFinite(occurredAt)) return false;
        if (
          selectedBounds.from &&
          occurredAt < new Date(selectedBounds.from).getTime()
        )
          return false;
        if (
          selectedBounds.to &&
          occurredAt > new Date(selectedBounds.to).getTime()
        )
          return false;
        return true;
      })
    : [];
  const demoNewSeats = isDemo
    ? demoContactsInScope.reduce(
        (total, contact) =>
          total +
          (contact.newSeatEvents || []).filter((value) => {
  ...
  const inSelectedPeriod = (value) => {
    ...
    if (
      ...
      return false;
    if (selectedBounds.to && occurredAt > new Date(selectedBounds.to).getTime())
      return false;
    return true;
  };
  const demoRenewedSubscribers = isDemo
    ? demoContactsInScope.filter((contact) => {
        if (contact.type !== "Abonado actual" || contact.isCommitmentOnly)
          return false;
        if (!selectedBounds.from && !selectedBounds.to) return true;
        return contact.renewalDate && inSelectedPeriod(contact.renewalDate);
      }).length
    : 0;
  const demoRenewedSeats = isDemo
    ? demoContactsInScope.reduce((total, contact) => {
        if (contact.type !== "Abonado actual" || contact.isCommitmentOnly)
          return total;
        const events = contact.renewedSeatEvents?.length
          ? contact.renewedSeatEvents
          : Array.from(
              { length: Number(contact.seats || 0) },
              () => contact.renewalDate,
            );
        return total + events.filter(inSelectedPeriod).length;
      }, 0)
    : 0;
  const demoMembershipSegments = isDemo
    ? demoContactsInScope.reduce((totals, contact) => {
        Object.entries(contact.membershipSegments || {}).forEach(
          ([segment, count]) => {
            totals[segment] = (totals[segment] || 0) + Number(count || 0);
          },
        );
        return totals;
      }, {})
  ...
  const summary = isDemo
    ? {
        ...
        currentSubscribers: demoContactsInScope.filter(
          (contact) =>
            ["Abonado actual", "Abonado nuevo"].includes(contact.type) &&
            !contact.isCommitmentOnly,
        ).length,
        activeSeats: demoContactsInScope
          .filter((contact) =>
            ["Abonado actual", "Abonado nuevo"].includes(contact.type),
          )
          .reduce((sum, contact) => sum + Number(contact.seats || 0), 0),
        renewing: demoContactsInScope.filter(
          (contact) => contact.type === "Por renovar",
        ).length,
        ...
        prospects: demoContactsInScope.filter(
          (contact) => contact.type === "Prospecto",
        ).length,
        membershipSegments: demoMembershipSegments,
        notContacted: demoContactsInScope.filter((contact) =>
          ["Sin contactar", "Por contactar"].includes(contact.stage),
        ).length,
        unassigned: demoContactsInScope.filter(
          (contact) => contact.executive === "SIN ASIGNAR",
        ).length,
        confirmedSales: dashboardSalesInScope.length,
        salesAmount: dashboardSalesInScope.reduce(
          (sum, sale) => sum + Number(sale.total || 0),
          0,
        ),
        collectedAmount: dashboardSalesInScope.reduce(
          (sum, sale) => sum + Number(sale.paid || 0),
          0,
        ),
      }
    : dashboardSummary || {};
  const salesTotal = isDemo
    ? dashboardSalesInScope.reduce(
        (sum, sale) =>
          sum + saleAmountForFacets(sale, reportFilters.purchaseFacets),
        0,
      )
    : Number(summary.salesAmount || 0);
  const totalContacts = Math.max(
    1,
    Number(summary.totalContacts || contacts.length || 1),
  );
  const funnelRows = [
    ["Contactos en alcance", Number(summary.totalContacts || 0), 100, "blue"],
    [
      "Abonados actuales",
      Number(summary.currentSubscribers || 0),
      (Number(summary.currentSubscribers || 0) / totalContacts) * 100,
      "green",
    ],
    [
      "Por renovar",
      Number(summary.renewing || 0),
      (Number(summary.renewing || 0) / totalContacts) * 100,
      "gold",
    ],
    [
      "Abonados nuevos",
      Number(summary.newSubscribers || 0),
      (Number(summary.newSubscribers || 0) / totalContacts) * 100,
      "violet",
    ],
    [
      "Prospectos",
      Number(summary.prospects || 0),
      (Number(summary.prospects || 0) / totalContacts) * 100,
      "gold",
    ],
  ];
  ...
  const subscriberMetricOptions = [
    ...
    { id: "renewed", label: "Renovados", value: summary.renewedSubscribers },
    {
      ...
      value:
        Number(summary.newSubscribers || 0) +
        Number(summary.renewedSubscribers || 0),
    },
  ];
  const seatMetricOptions = [
    ...
    { id: "renewed", label: "Renovados", value: summary.renewedSeats },
    {
      id: "total",
      label: "N + R",
      value: Number(summary.newSeats || 0) + Number(summary.renewedSeats || 0),
    },
  ];
  let segmentCursor = 0;
  const segmentGradient = segmentTotal
    ? `conic-gradient(${segmentRows
        .map(([, value, color]) => {
        ...
        .join(", ")})`
    : "conic-gradient(#d8dde4 0 100%)";
  const operationCounts = followupCounts || {
    scheduled: tasks.length,
    pending: tasks.filter(
      (task) => task.status === "Pendiente" || task.status === "En curso",
    ).length,
    completed: tasks.filter((task) => task.status === "Completada").length,
    overdue: tasks.filter((task) => task.status === "Vencida").length,
  };
  const filterKey = `${reportFilters.season}|${reportFilters.period}|${reportFilters.fromDate}|${reportFilters.toDate}|${reportFilters.executiveId}|${(reportFilters.purchaseFacets || []).join(",")}`;
  const reportIsReady =
    Boolean(dashboardSummary) && (isDemo || loadedFilterKey === filterKey);
  const executiveName =
    availableExecutives.find((item) => item.id === reportFilters.executiveId)
      ?.displayName || "Todos los ejecutivos";
  useEffect(() => {
    ...
    onLoadDashboard({
      ...
      purchaseFacets: reportFilters.purchaseFacets,
      ...periodBounds(
        reportFilters.period,
        reportFilters.fromDate,
        reportFilters.toDate,
      ...
      .then((applied) => {
        if (active && applied !== false) setLoadedFilterKey(filterKey);
      })
      .catch((error) => {
        if (active)
          onNotify(error.message || "No fue posible actualizar el reporte.");
      });
    return () => {
      active = false;
    };
  ...
  async function downloadSubscriberDetail() {
    ...
    try {
      ...
      setPdfState({
        status: "success",
        message: "Reporte detallado descargado.",
      });
    } catch (error) {
      setPdfState({
        status: "error",
        message:
          error.message || "No fue posible descargar el reporte detallado.",
      });
    }
  }
  ...
  const reportActions = (
    <>
      <span className="updated-badge">
        <span />
        {reportIsReady ? "Periodo actualizado" : "Actualizando periodo…"}
      </span>
      {reportIsReady && (
        <details className="report-download-menu">
          ...
          >
            ...
            Descargar <Icon name="chevron" size={14} />
          </summary>
          <div className="report-download-options">
            ...
            >
              ...
              <span>
                <strong>Reporte ejecutivo PDF</strong>
                <small>Resumen visual del periodo</small>
              </span>
            </button>
            {canExportData(user) && (
              <button
                type="button"
                disabled={pdfState.status === "generating"}
                onClick={downloadSubscriberDetail}
              >
                <Icon name="people" size={17} />
                <span>
                  <strong>Detalle de titulares</strong>
                  <small>Zonas, butacas y ordenes vigentes</small>
                </span>
              </button>
            )}
          </div>
      ...
      {pdfState.message && (
        <span
          className={`pdf-status pdf-status--${pdfState.status}`}
  ...
  return (
    <div className="page-wrap">
      <PageHeader
        ...
        title="Reporte Dirección"
        description={
          <>
            Seguimiento de venta y renovación de abonados{" "}
            <strong className="charros-name">Charros de Jalisco</strong>
          </>
        }
      ...
      <section className="metrics-grid" aria-label="Indicadores principales">
        <MetricCard
          label="Abonados actuales"
          value={integer.format(summary.currentSubscribers || 0)}
          detail="Titulares Identificados"
          icon="people"
        />
        <MetricCard
          label="Abonos Activos"
          value={integer.format(summary.activeSeats || 0)}
          detail="Butacas Individuales"
          icon="layers"
          tone="violet"
        />
        <MetricCard
          label="Por renovar"
          value={integer.format(summary.renewing || 0)}
          detail="Personas por renovar sus abonos"
          icon="refresh"
          tone="gold"
        />
        <SegmentedMetricCard
          label="Titulares"
          options={subscriberMetricOptions}
          selected={subscriberMetricMode}
          onSelect={setSubscriberMetricMode}
          detail={`Periodo: ${newSubscriberPeriodLabel}`}
          icon="chart"
          tone="green"
        />
        <SegmentedMetricCard
          ...
          selected={seatMetricMode}
          onSelect={setSeatMetricMode}
          detail={`Butacas individuales · Periodo: ${newSubscriberPeriodLabel}`}
          icon="layers"
          tone="violet"
        />
        <MetricCard
          label="Venta documentada"
      ...
      <section className="dashboard-grid">
        <article className="panel panel--wide">
          <div className="panel-heading">
            <div>
              <span className="panel-kicker">COMPOSICIÓN OPERATIVA</span>
              <h2>Foto actual de la cartera</h2>
            </div>
            <span className="small-chip">
              {integer.format(summary.totalContacts || 0)} registros
            </span>
          </div>
        ...
        <article className="panel operation-card">
          <div className="panel-heading">
            <div>
              <span className="panel-kicker">OPERACIÓN GLOBAL DE HOY</span>
              <h2>Seguimientos</h2>
            </div>
            <button
            ...
            >
              Ver operación <Icon name="arrow" size={15} />
            </button>
          </div>
          <div className="operation-total">
            <strong>{operationCounts.scheduled}</strong>
            <span>acciones programadas</span>
            <small>operación del día</small>
          </div>
          <div className="operation-stats">
            <div>
              <span className="operation-dot operation-dot--blue" />
              <strong>{operationCounts.pending}</strong>
              <small>Pendientes</small>
            </div>
            <div>
              <span className="operation-dot operation-dot--green" />
              <strong>{operationCounts.completed}</strong>
              <small>Completadas</small>
            </div>
            <div>
              <span className="operation-dot operation-dot--red" />
              <strong>{operationCounts.overdue}</strong>
              <small>Vencidas</small>
            </div>
          </div>
          {tasks[0] ? (
            <div className="next-action">
              <span className="mini-avatar">{initials(tasks[0].contact)}</span>
              <div>
                <small>Siguiente · {tasks[0].time}</small>
                <strong>{tasks[0].contact}</strong>
                <span>{tasks[0].action}</span>
              </div>
            ...
            <div className="next-action">
              ...
              <div>
                <small>Agenda al día</small>
                <strong>Sin acciones pendientes</strong>
                <span>No hay acciones programadas para este corte.</span>
              </div>
            </div>
          )}
        </article>
...
function ContactsPage({
  ...
  const serverQuery = useMemo(
    () => ({
      ...
      subscriberStatus:
        status === "Todos" ? undefined : subscriberStatusCode(status),
      commercialStage:
        stage === "Todas" ? undefined : commercialStageCode(stage),
      assignment:
        owner === "__unassigned__"
          ...
          : owner === "__assigned__"
            ? "assigned"
            : undefined,
      executiveId: !["Todos", "__unassigned__", "__assigned__"].includes(owner)
        ? owner
        : undefined,
  ...
  useEffect(() => {
    ...
    onLoadContacts(serverQuery)
      .then((nextMeta) => {
        if (active)
          setMeta({ page: 1, pageSize, total: 0, totalPages: 1, ...nextMeta });
      })
      .catch((error) => {
        if (active)
          setLoadError(error.message || "No fue posible cargar los contactos.");
      })
      .finally(() => {
  ...
  const source = contacts.filter(
    (contact) =>
      (isPortfolio || contact.kind === kind) &&
      Boolean(contact.deletedAt) === showDeleted,
  );
  const filtered = isDemo
    ? source
        .filter((contact) => {
          const query = search.toLowerCase();
          const ownerName = availableExecutives.find(
            (item) => item.id === owner,
          )?.displayName;
          return (
            (!query ||
            ...
            (owner === "Todos" ||
              (owner === "__unassigned__" &&
                contact.executive === "Sin asignar") ||
              contact.executive === ownerName) &&
            (!channel ||
              contact.lastHumanContactChannel === channel ||
              {
                Llamada: "phone",
                WhatsApp: "whatsapp",
                Correo: "email",
        ...
        .sort((left, right) => {
          ...
          return (
            String(values[0]).localeCompare(String(values[1]), "es-MX") *
            (order === "asc" ? 1 : -1)
          );
        })
    : source;
  ...
  const stats = isPortfolio
    ? [
        ["Registros en página", filtered.length],
        [
          "Abonados actuales",
          filtered.filter((item) => item.type === "Abonado actual").length,
        ],
        [
          "Por renovar",
          filtered.filter((item) => item.type === "Por renovar").length,
        ],
        [
          "Sin siguiente tarea",
          filtered.filter((item) => item.nextTask === "Sin tarea").length,
        ],
      ]
    : [
        ["Registros en página", filtered.length],
        [
          "Contactados",
          filtered.filter((item) => item.stage === "Contactado").length,
        ],
        [
          "Con interés",
          filtered.filter((item) => item.stage === "Interesado").length,
        ],
        [
          "Sin asignar",
          filtered.filter((item) => item.executive === "Sin asignar").length,
        ],
      ];
  ...
  function toggleSort(field) {
    if (sort === field)
      ...
      setSort(field);
      setOrder(field === "name" || field === "status" ? "asc" : "desc");
    }
  }
  ...
  function applyQuickView(view) {
    ...
    if (view === "Hoy") {
      const today = localInputDate(new Date());
      setFrom(today);
      setTo(today);
      setDateField("nextFollowUp");
    }
    if (view === "Vencidos") {
      setFrom("");
      setTo(localInputDate(new Date()));
      setDateField("nextFollowUp");
    }
  ...
  return (
    <div className="page-wrap">
      <PageHeader
        ...
        description={
          isPortfolio
            ? "Personas, abonos y seguimiento comercial sin duplicar información."
            : "Convierte interés en oportunidades con un siguiente paso siempre visible."
        }
        actions={
          <>
            {canExportData(user) && !showDeleted && (
              <SecondaryButton
                icon="download"
                onClick={() =>
                  onExport({
            ...
            {canCreateContacts(user) && (
              <PrimaryButton onClick={() => onCreate(kind)}>
                Nuevo {isPortfolio ? "contacto" : "prospecto"}
              </PrimaryButton>
            )}
          </>
        }
      ...
      >
        {stats.map(([label, value], index) => (
          <div key={label} className={index === 3 ? "stat-alert" : ""}>
            <span>{label}</span>
            <strong>{value}</strong>
          </div>
        ))}
      ...
      <section className="panel list-panel" aria-busy={loading}>
        <div className="toolbar">
          <label className="search-field">
            ...
            <input
              ...
              placeholder="Buscar por nombre, teléfono, correo, ID u observaciones…"
            />
          </label>
          <div className="toolbar-filters">
            <label>
              ...
              >
                ...
                {isPortfolio ? (
                  <>
                    <option>Abonado actual</option>
                    <option>Por renovar</option>
                    <option>Abonado nuevo</option>
                    <option>Exabonado</option>
                  </>
                ) : (
            ...
            <label>
              ...
              >
                <option>Todos</option>
                <option value="__assigned__">Con ejecutivo</option>
                <option value="__unassigned__">Sin asignar</option>
                {!availableExecutives.some((item) => item.id === user.id) &&
                  user.roleCode === "executive" && (
                    <option value={user.id}>{user.name}</option>
                  )}
                {availableExecutives.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.displayName}
                  </option>
                ))}
            ...
            <label>
              <span className="sr-only">Canal del último contacto</span>
              <select
                value={channel}
            ...
            {(from || to) && (
              <label>
                <span className="sr-only">Campo de fecha</span>
                <select
                  value={dateField}
        ...
        <div className="active-filters">
          ...
          {[
            ...
            <button key={view} onClick={() => applyQuickView(view)}>
              {view}
            </button>
          ))}
          {canRestoreContacts(user) && (
            <button
              className={showDeleted ? "quick-view-active" : ""}
          ...
          {(search ||
            ...
            <button className="clear-filter" onClick={clearFilters}>
              Limpiar filtros <Icon name="close" size={13} />
            </button>
          )}
        </div>
        {loadError && (
          <div className="load-error" role="alert">
            ...
            <span>{loadError}</span>
            <button onClick={() => setReloadToken((current) => current + 1)}>
              Reintentar
            </button>
          </div>
        )}
        <div className="table-scroll">
          ...
          {!loading && !filtered.length && (
            <EmptyState
              ...
              body="Prueba quitando uno o más filtros de la lista."
            />
          )}
        </div>
        <div className="table-footer">
          <span>
            Mostrando {filtered.length} de {integer.format(total)} registros
          </span>
...
function ContactRow({ contact, isPortfolio, onEdit }) {
  return (
    <tr>
      ...
      <td>
        <StatusPill>{contact.type}</StatusPill>
      </td>
      {isPortfolio && (
        <td>
      ...
      <td>
        <StatusPill>{contact.stage}</StatusPill>
      </td>
      {isPortfolio && (
        <td>
          <strong>{contact.seasons || contact.declaredSeasons || "—"}</strong>
          {contact.seasons > 0 ? (
            <small className="subvalue">verificadas</small>
          ) : contact.declaredSeasons != null ? (
            <small className="subvalue">declaradas</small>
          ) : null}
        </td>
      )}
      <td>
      ...
      <td>
        <div className="owner-cell">
          {contact.executive !== "Sin asignar" && (
            <span className="owner-avatar">{initials(contact.executive)}</span>
          )}
      ...
      <td>
        <span
          className={contact.note ? "note-preview" : "muted"}
          title={contact.note || undefined}
        >
...
function MembershipCell({ contact, onEdit }) {
  ...
  const seats =
    membership?.units?.map((unit) => unit.seatIdentifier).filter(Boolean) || [];
  const action = membership ? "Editar" : "Agregar";
  return (
    <div className="membership-cell">
      {membership ? (
        <>
          <strong>{membership.localityName || "Localidad pendiente"}</strong>
          <small>
            {membership.membershipSection || "Sección pendiente"} ·{" "}
            {membership.seatCount}{" "}
            {membership.seatCount === 1 ? "abono" : "abonos"}
          </small>
          <small title={seats.join(", ") || undefined}>
            {seats.length ? seats.join(", ") : "Butacas pendientes"}
          </small>
          {membership.netAmount != null && (
            <>
              <span className="membership-net-value">
                Importe neto: {currency.format(membership.netAmount)}
              </span>
              <span className="membership-commercial-value">
                Valor comercial: {currency.format(membership.commercialValue)} ·
                Descuento: {currency.format(membership.discountAmount || 0)}
              </span>
            </>
          )}
        </>
        ...
        <>
          <strong>
            {contact.seats}{" "}
            {contact.seats === 1 ? "abono registrado" : "abonos registrados"}
          </strong>
          <small>Abre para consultar el detalle</small>
        </>
      ) : (
        <>
          <strong>Sin capturar</strong>
      ...
      <button
        type="button"
        onClick={() => onEdit(contact, { focusMembership: true })}
        aria-label={`${action} abonos de ${contact.name}`}
      >
        {action}
      </button>
    </div>
...
function FollowupPage({
  ...
  const visibleTasks =
    tab === "overdue"
      ? tasks.filter((task) => task.status === "Vencida")
      : tasks;
  ...
  return (
    <div className="page-wrap">
      <PageHeader
        ...
        description="Tareas, interacciones y excepciones reunidas para que nada se quede sin atender."
      />
      <section className="operation-summary">
        ...
        <div>
          <span className="summary-icon summary-icon--gold">
            <Icon name="people" />
          </span>
          <span>
            <small>Sin asignar</small>
            <strong>{exactUnassignedCount}</strong>
          </span>
          <em>Distribuir cartera</em>
        </div>
      </section>
      <section className="panel followup-panel">
        ...
        >
          ...
          >
            Mis tareas <span>{pendingCount}</span>
          </button>
          <button
            role="tab"
            aria-selected={tab === "log"}
            className={tab === "log" ? "tab-active" : ""}
            onClick={() => setTab("log")}
          >
            Bitácora
          </button>
          <button
            ...
            className={tab === "overdue" ? "tab-active" : ""}
            onClick={() => setTab("overdue")}
          >
            Vencidos y sin asignar{" "}
            <span className="tab-alert">
              {overdueCount + exactUnassignedCount}
            </span>
          </button>
        </div>
        {tab === "log" ? (
...
function TaskList({ tasks, overdue, unassigned, user, onCompleteTask }) {
  ...
  async function complete(task) {
    ...
    try {
      ...
      /* El contenedor muestra el error global. */
    } finally {
      setUpdatingTaskId("");
    }
  }
  function mayComplete(task) {
    ...
    return (
      hasPermission(user, PERMISSIONS.TASK_WRITE_ASSIGNED) &&
      task.assignedTo === user.id
    );
  }
  return (
    <>
      <div className="toolbar followup-toolbar">
          ...
          <tbody>
            {tasks.map((task) => (
              <tr key={task.id}>
                <td>
                  <strong
                    className={task.status === "Vencida" ? "danger-text" : ""}
                  >
                    {task.time}
                  </strong>
                </td>
                ...
                <td>
                  {mayComplete(task) && (
                    ...
                    >
                      ...
                      {updatingTaskId === task.id ? "Guardando…" : "Completar"}
                    </button>
                  )}
                </td>
              </tr>
            ...
            {overdue &&
              unassigned.map((contact) => (
                <tr key={contact.id}>
                  <td>
                    <strong className="danger-text">Sin fecha</strong>
                  </td>
                  <td>
                    <div className="person-cell">
                      <span className="mini-avatar">{contact.initials}</span>
                      <strong>{contact.name}</strong>
                    </div>
                  </td>
                  <td>Asignar responsable y siguiente paso</td>
                  <td className="danger-text">Sin asignar</td>
                  <td>
                    <span className="priority priority--alta">Alta</span>
                  </td>
                  <td>
                    <StatusPill tone="danger">Requiere atención</StatusPill>
                  </td>
                  <td />
                </tr>
              ))}
  ...
  return (
    <div className="interaction-layout">
      <div className="timeline">
        {interactions.map((item) => (
          <article className="timeline-item" key={item.id}>
            <div className="timeline-marker">
              <Icon
                name={item.type === "Correo" ? "mail" : "phone"}
                size={16}
              />
            </div>
        ...
        {!interactions.length && (
          <EmptyState
            title="La bitácora global aún no está cargada"
            body="Abre un contacto para consultar sus interacciones o registra la primera gestión."
          />
        )}
      </div>
      <aside className="activity-note">
        <span className="summary-icon summary-icon--blue">
          <Icon name="note" />
        </span>
        <h3>Una bitácora confiable</h3>
        <p>
          Los envíos de campaña se conservan por separado. Solo llamadas,
          mensajes con respuesta y gestiones del equipo actualizan el último
          contacto humano.
        </p>
      </aside>
    </div>
...
function initialSaleDraft() {
  return {
    ...
    contactId: "",
    executiveId: "",
    commercialCategory: "subscription",
    coverageSeasons: 1,
    suiteNumber: "",
    commitmentTotal: "",
    parkingQuantity: 0,
...
function SalesPage({
  ...
  const isCommitmentDraft = saleDraft.commercialCategory === "commitment";
  ...
  const contactOptions = [
    ...new Map([
      ...
      ...sales
        .filter((sale) => sale.contactId)
        .map((sale) => [
          sale.contactId,
          { id: sale.contactId, name: sale.contact },
        ]),
      ...sales.flatMap((sale) =>
        (sale.holderAssignments || []).map((holder) => [
          holder.contactId,
          {
            id: holder.contactId,
            name: holder.contactName || "Titular asociado",
          },
        ]),
      ),
    ]).values(),
  ...
  useEffect(() => {
    ...
    setSaleDraft((current) => ({
      ...
      kind:
        saleClosure.contact.type === "Por renovar" ||
        saleClosure.contact.type === "Abonado actual"
          ? "renewal"
          : "new",
      closeStage: saleClosure.stage === "Ganado" ? "won" : "reserved",
      zone:
        saleClosure.contact.zone === "Sin definir"
          ? ""
          : saleClosure.contact.zone || "",
  ...
  const owners = [
    ...new Set(sales.map((sale) => sale.owner).filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b, "es"));
  const kinds = [
    ...new Set(
      sales.map((sale) => sale.kind).filter((value) => value && value !== "—"),
    ),
  ].sort((a, b) => a.localeCompare(b, "es"));
  const filteredSales = sales.filter((sale) => {
    const query = search.trim().toLowerCase();
    const occurredAt = sale.occurredAt
      ? new Date(sale.occurredAt).getTime()
      : NaN;
    return (
      (!query ||
        `${sale.externalOrderNumber || ""} ${sale.id} ${sale.contact} ${sale.zone}`
          .toLowerCase()
          .includes(query)) &&
      ...
      (!segments.length ||
        segments.some((value) =>
          (sale.purchaseFacets || [sale.segment]).includes(value),
        )) &&
      (!from ||
        (Number.isFinite(occurredAt) &&
          occurredAt >= new Date(`${from}T00:00:00`).getTime())) &&
      (!to ||
        (Number.isFinite(occurredAt) &&
          occurredAt <= new Date(`${to}T23:59:59.999`).getTime()))
    );
  });
  ...
  const seats = metricSales.reduce(
    (sum, sale) => sum + Number(sale.seats || 0),
    0,
  );
  const partialCount = metricSales.filter(
    (sale) => sale.status === "Parcial",
  ).length;
  ...
  const draftDocumentedTotal =
    (isCommitmentDraft
      ? Number(saleDraft.commitmentTotal || 0)
      : (saleQuote?.netAmount ?? draftChargedUnits * draftUnitPrice)) +
    Number(saleDraft.parkingQuantity || 0) * 4640;
  useEffect(() => {
    ...
    if (
      isCommitmentDraft ||
      !saleDraft.localityCode ||
      !saleDraft.discountCode ||
    ...
    const timeout = window.setTimeout(async () => {
      try {
        const quote = await onQuoteMembershipPricing({
          localityCode: saleDraft.localityCode,
          discountCode: saleDraft.discountCode,
          seatCount: draftQuantity,
        });
        ...
        setSaleDraft((current) => ({
          ...current,
          unitPrice:
            quote.pricingMode === "two_for_one"
              ? quote.netAmount / quote.chargedUnits
              : quote.netAmount / draftQuantity,
          promotion2x1: quote.pricingMode === "two_for_one",
        }));
        ...
        setSaleQuoteError(
          error?.message || "No fue posible calcular el descuento.",
        );
      }
    }, 250);
    return () => window.clearTimeout(timeout);
  ...
  function openSaleCorrection(sale) {
    ...
    const subscriptionItems = items.filter(
      (item) =>
        !/estacionamiento/i.test(`${item.product || ""} ${item.zone || ""}`),
    );
    const chargedItem =
      subscriptionItems.find((item) => Number(item.unitPrice || 0) > 0) ||
      subscriptionItems[0] ||
      {};
    const rawZone =
      chargedItem.zone ||
    ...
    setSaleDraft({
      ...
      commercialCategory:
        sale.commercialCategory ||
        (sale.segment === "Compromisos" ? "commitment" : "subscription"),
      coverageSeasons: Number(
        sale.coverageSeasons || (sale.segment === "Compromisos" ? 2 : 1),
      ),
      suiteNumber: sale.suiteNumber || "",
      commitmentTotal:
        sale.segment === "Compromisos"
          ? String(
              Number(sale.total || 0) -
                Number(sale.parkingQuantity || 0) * 4640,
            )
          : "",
      parkingQuantity: Number(sale.parkingQuantity || 0),
      kind: sale.saleType || (sale.kind === "Renovación" ? "renewal" : "new"),
      closeStage:
        sale.status === "reserved" || sale.commercialStatus === "Apartada"
          ? "reserved"
          : "won",
      ...
      notes: sale.notes || "",
      correctionReason: "",
      additionalHolders: (sale.holderAssignments || [])
        .filter((holder) => !holder.isPrimary)
        .map((holder) => ({
          contactId: holder.contactId,
          quantity: Number(holder.quantity),
        })),
      seatDetails: (sale.holderAssignments || [])
        .flatMap((holder) => holder.seatDetails || [])
        .map((seat, index) => ({ ...seat, unitNumber: index + 1 })),
    });
    setSaleError("");
    setSaleOpen(true);
  }
  async function submitPayment(event) {
    ...
    setPaymentError("");
    try {
      await onAddPayment(paymentSale, {
        ...paymentDraft,
        amount,
        paidAt: new Date(`${paymentDraft.paidAt}T12:00:00`).toISOString(),
      });
      setPaymentSale(null);
    } catch (error) {
      setPaymentError(error.message || "No fue posible registrar el cobro.");
    } finally {
  ...
  async function submitSale(event) {
    ...
    const paymentAmount = Number(saleDraft.paymentAmount || 0);
    const promotion2x1 = saleQuote?.pricingMode === "two_for_one";
    const items = isCommitmentDraft
      ? buildCommitmentItems({
          suiteNumber: saleDraft.suiteNumber,
          quantity,
          totalAmount: saleDraft.commitmentTotal,
          coverageSeasons: saleDraft.coverageSeasons,
        })
      : buildSaleItems({
          kind: saleDraft.kind,
          zone: saleDraft.zone,
          ...
          unitPrice,
          promotion2x1,
          discountCode: saleQuote?.discountCode,
          discountName: saleQuote?.discountName,
          pricingMode: saleQuote?.pricingMode,
          chargedUnits: saleQuote?.chargedUnits,
          bonusUnits: saleQuote?.bonusUnits,
        });
    const parkingQuantity = Number(saleDraft.parkingQuantity || 0);
    const totalAmount =
      items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0) +
      parkingQuantity * 4640;
    const additionalHolderQuantity = saleDraft.additionalHolders.reduce(
      (sum, holder) => sum + Number(holder.quantity || 0),
      0,
    );
    const holderIds = [
      saleDraft.contactId,
      ...saleDraft.additionalHolders.map((holder) => holder.contactId),
    ];
    const invalidHolderDistribution =
      saleDraft.additionalHolders.some(
        (holder) =>
          !holder.contactId ||
          !Number.isInteger(Number(holder.quantity)) ||
          Number(holder.quantity) < 1,
      ) ||
      new Set(holderIds).size !== holderIds.length ||
      additionalHolderQuantity >= quantity;
    const invalidCommercialTerms = isCommitmentDraft
      ? !saleDraft.suiteNumber.trim() ||
        Number(saleDraft.coverageSeasons) !== 2 ||
        !Number.isFinite(Number(saleDraft.commitmentTotal)) ||
        Number(saleDraft.commitmentTotal) < 0 ||
        !items.length
      : !saleDraft.localityCode ||
        !saleDraft.discountCode ||
        saleQuoteState !== "ready" ||
        !saleQuote ||
    ...
    if (
      ...
      !saleDraft.contactId ||
      !saleDraft.executiveId ||
      !Number.isInteger(quantity) ||
      quantity < 1 ||
      !Number.isInteger(parkingQuantity) ||
      parkingQuantity < 0 ||
      parkingQuantity > 100 ||
    ...
    try {
      const holderAssignments = [
        {
          contactId: saleDraft.contactId,
          quantity: quantity - additionalHolderQuantity,
          isPrimary: true,
        },
        ...saleDraft.additionalHolders.map((holder) => ({
          contactId: holder.contactId,
          quantity: Number(holder.quantity),
          isPrimary: false,
        })),
      ...
      const payload = {
        ...
        coverageSeasons: Number(saleDraft.coverageSeasons),
        suiteNumber: isCommitmentDraft
          ? saleDraft.suiteNumber.trim()
          : undefined,
        saleType: saleDraft.kind,
        closeStage: saleDraft.closeStage,
        contactId: saleDraft.contactId,
        executiveId: saleDraft.executiveId,
        seasonCode: "LMP-2026-27",
        status: saleDraft.closeStage === "won" ? "confirmed" : "reserved",
        soldAt: new Date(`${saleDraft.soldAt}T12:00:00`).toISOString(),
        currency: "MXN",
        notes:
          [
            promotion2x1
              ? "Promoción 2x1 aplicada automáticamente desde el catálogo oficial."
              : "",
            saleDraft.notes,
          ]
            .filter(Boolean)
      ...
      if (editingSale)
        await onCorrectSale(editingSale, {
          ...payload,
          reason: saleDraft.correctionReason.trim(),
        });
      else
        await onCreateSale({
          ...payload,
              ...
              ? [
                  {
                    ...
                    method: saleDraft.paymentMethod,
                    paidAt: new Date(
                      `${saleDraft.soldAt}T12:00:00`,
                    ).toISOString(),
                    reference: saleDraft.paymentReference || undefined,
                  },
                ]
              : [],
        });
      ...
      setSaleError(error.message || "No fue posible crear la venta.");
    } finally {
      setSavingSale(false);
    }
  }
  ...
  async function submitCancellation(event) {
    ...
    if (reason.length < 5) {
      setCancellationError(
        "Describe el motivo de la anulación con al menos 5 caracteres.",
      );
      return;
    }
  ...
  return (
    <div className="page-wrap">
      <PageHeader
        ...
        title="Ventas"
        description="Movimientos, pagos y abonos vinculados al contacto responsable."
        actions={
      ...
      <section className="sales-metrics">
        <div>
          <span>Venta en corte cargado</span>
          <strong>{currency.format(total)}</strong>
          <small>Temporada LMP 2026–2027</small>
        </div>
        <div>
          <span>Cobrado en el corte</span>
          <strong>{currency.format(paid)}</strong>
          <small>{collectedPercentage}% del total cargado</small>
        </div>
        <div>
          <span>Saldo en el corte</span>
          <strong>{currency.format(Math.max(0, total - paid))}</strong>
          <small>{partialCount} movimientos parciales</small>
        </div>
        <div>
          <span>Abonos asociados</span>
          <strong>{seats}</strong>
          <small>{filteredSales.length} movimientos cargados</small>
        </div>
      </section>
        ...
        <div className="toolbar">
          <label className="search-field">
            ...
            <input
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Buscar orden, contacto, suite o zona…"
            />
          </label>
          <div className="toolbar-filters">
            <select
        ...
        <div className="table-scroll">
            ...
            <tbody>
              {filteredSales.map((sale) => (
                <tr key={sale.id}>
                  ...
                  <td>{sale.zone}</td>
                  <td>{sale.coverageLabel}</td>
                  <td>{sale.seats}</td>
                  <td>
                    <strong>{currency.format(sale.total)}</strong>
                  </td>
                  <td>{currency.format(sale.paid)}</td>
                  <td>{sale.owner}</td>
                  <td>
          ...
          {!filteredSales.length && (
            <EmptyState
              ...
              body="No hay ventas que coincidan con los filtros seleccionados."
            />
          )}
        </div>
        <div className="table-footer">
          <span>
            {filteredSales.length} de {sales.length} movimientos cargados
          </span>
          <small>
            Los indicadores y la tabla responden a los filtros. Las anulaciones,
            correcciones y pagos conservan historial de auditoría.
          </small>
        </div>
      </section>
      {cancellingSale && (
        <div className="drawer-backdrop" role="presentation">
          ...
          >
            <header className="drawer-header">
              <div>
                <span className="eyebrow">Anulación auditada</span>
                <h2 id="cancel-sale-title">
                  Anular orden{" "}
                  {cancellingSale.externalOrderNumber || cancellingSale.id}
                </h2>
                <p>
                  Dejará de contar en Ventas, Abonos activos y Segmentación. El
                  historial original permanecerá disponible para auditoría.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitCancellation}>
              <div className="drawer-body">
                <label className="field">
                  ...
                  <textarea
                    ...
                    placeholder="Ej. Registro duplicado del mismo cliente."
                  />
                </label>
                {cancellationError && (
                  <p className="manual-submit-error" role="alert">
                    {cancellationError}
                  </p>
                )}
              </div>
              <footer className="drawer-footer">
                ...
                <button
                  ...
                  disabled={savingCancellation}
                >
                  {savingCancellation ? "Anulando…" : "Confirmar anulación"}
                </button>
              </footer>
            </form>
          </aside>
      ...
      {saleOpen && (
        <div className="drawer-backdrop" role="presentation">
          <aside
            className="drawer drawer--manual"
            role="dialog"
            aria-modal="true"
            ...
            <header className="drawer-header">
              <div>
                <span className="eyebrow">
                  {editingSale ? "Corrección auditada" : "Alta guiada"}
                </span>
                <h2 id="sale-title">
                  {editingSale ? "Corregir venta" : "Nueva venta"}
                </h2>
                <p>
                  {editingSale
                    ? "Modifica los datos comerciales. La venta original y sus cobros se conservarán en el historial."
                    : "La orden, sus abonos y el cobro inicial quedarán vinculados al titular."}
                </p>
              </div>
              <button
                type="button"
                ...
                <div className="form-grid">
                  <label className="field">
                    ...
                    <input
                      ...
                      required
                      maxLength="80"
                      value={saleDraft.externalOrderNumber}
                      onChange={(event) =>
                        setSaleDraft((current) => ({
                          ...current,
                    ...
                    />
                    <small>Identificador único de la venta.</small>
                  </label>
                  <label className="field">
                    <span>Cierre *</span>
                    <select
                      value={saleDraft.closeStage}
                      onChange={(event) =>
                        setSaleDraft((current) => ({
                          ...current,
                          closeStage: event.target.value,
                        }))
                      }
                    >
                  ...
                  <label className="field field--full">
                    ...
                    />
                    <small>
                      Filtra el acervo completo de contactos, incluidos
                      prospectos.
                    </small>
                  </label>
                  <label className="field field--full">
                    ...
                    <small>
                      ...
                      <button
                        ...
                        onClick={() => {
                          setSaleOpen(false);
                          onClearSaleClosure();
                          onCreate("prospect", { resumeSale: true });
                        }}
                      >
                        Crear contacto nuevo y continuar la venta
                      </button>
                    </small>
                  </label>
                  <details className="field field--full holder-distribution">
                    <summary>
                      Distribuir la orden entre varios titulares
                    </summary>
                    <p>
                      Opcional. Si no agregas personas, todos los abonos
                      pertenecerán al titular principal.
                    </p>
                    <div className="holder-primary">
                      <strong>Titular principal</strong>
                      <span>
                        {Math.max(
                          0,
                          Number(saleDraft.quantity || 0) -
                            saleDraft.additionalHolders.reduce(
                              (sum, holder) =>
                                sum + Number(holder.quantity || 0),
                              0,
                            ),
                        )}{" "}
                        abonos
                    ...
                    {saleDraft.additionalHolders.map((holder, index) => (
                      <div className="holder-row" key={`holder-${index}`}>
                        <label className="field">
                          <span>Titular adicional *</span>
                          <select
                            value={holder.contactId}
                            onChange={(event) =>
                              setSaleDraft((current) => ({
                                ...current,
                                additionalHolders:
                                  current.additionalHolders.map(
                                    (item, itemIndex) =>
                                      itemIndex === index
                                        ? {
                                            ...item,
                          ...
                          >
                            ...
                            {contactOptions
                              .filter(
                                (contact) => contact.id !== saleDraft.contactId,
                              )
                              .map((contact) => (
                                <option key={contact.id} value={contact.id}>
                                  {contact.name}
                        ...
                        <button
                          ...
                          onClick={() =>
                            setSaleDraft((current) => ({
                              ...current,
                              additionalHolders:
                                current.additionalHolders.filter(
                    ...
                    <button
                      type="button"
                      className="button button--secondary"
                      disabled={Number(saleDraft.quantity || 0) < 2}
                      onClick={() =>
                        setSaleDraft((current) => ({
                          ...current,
                          additionalHolders: [
                            ...current.additionalHolders,
                  ...
                  <label className="field">
                    ...
                    <select
                      value={saleDraft.commercialCategory}
                      onChange={(event) =>
                        setSaleDraft((current) => ({
                          ...current,
                          commercialCategory: event.target.value,
                          coverageSeasons:
                            event.target.value === "commitment" ? 2 : 1,
                          suiteNumber:
                          ...
                          commitmentTotal:
                            event.target.value === "commitment"
                              ? current.commitmentTotal
                              : "",
                        }))
                      }
                    >
                      <option value="subscription">Abono de temporada</option>
                      <option value="commitment">
                        Compromiso anual de suite
                      </option>
                    </select>
                  </label>
                  {!isCommitmentDraft && (
                    <label className="field">
                      ...
                      >
                        <option value="new">Abono nuevo</option>
                  ...
                  <label className="field">
                    <span>Ejecutivo *</span>
                    <select
                      value={saleDraft.executiveId}
                      onChange={(event) =>
                        setSaleDraft((current) => ({
                          ...current,
                          executiveId: event.target.value,
                        }))
                      }
                    >
                  ...
                  {isCommitmentDraft ? (
                    <>
                      <label className="field">
                        <span>Número de suite *</span>
                        <input
                          required
                          maxLength="40"
                          value={saleDraft.suiteNumber}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                              suiteNumber: event.target.value,
                            }))
                          }
                          placeholder="Ej. 14"
                        />
                        <small>
                          Se guarda como identificador de la suite dentro de la
                          orden.
                        </small>
                      </label>
                      <label className="field">
                        ...
                        <select
                          value={saleDraft.coverageSeasons}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                              coverageSeasons: Number(event.target.value),
                            }))
                          }
                        >
                          <option value="2">Anual · 2 temporadas</option>
                        </select>
                        <small>
                          Comprende la temporada LMP y la temporada
                          complementaria del año.
                        </small>
                      </label>
                      <label className="field">
                        <span>Número de butacas *</span>
                        <input
                          type="number"
                          min="1"
                          max="100"
                      ...
                      <label className="field">
                        <span>Valor anual comprometido *</span>
                        <input
                          type="number"
                          min="0"
                          step="0.01"
                          value={saleDraft.commitmentTotal}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                              commitmentTotal: event.target.value,
                            }))
                          }
                        />
                        <small>
                          Importe completo de la suite por las dos temporadas.
                          Se suma a venta documentada; el cobro inicial se
                      ...
                      <label className="field">
                        ...
                        <input
                          ...
                          min="0"
                          max="100"
                          value={saleDraft.parkingQuantity}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                              parkingQuantity: event.target.value,
                            }))
                          }
                        />
                        <small>
                          Opcional · {currency.format(4640)} por lugar y por
                          temporada.
                        </small>
                      </label>
                    </>
                    ...
                    <>
                      <label className="field">
                        <span>Zona *</span>
                        <select
                          value={saleDraft.localityCode}
                          onChange={(event) => {
                            const locality = saleLocalities.find(
                              (item) => item.code === event.target.value,
                            );
                            setSaleDraft((current) => ({
                              ...current,
                              localityCode: event.target.value,
                              zone: locality?.displayName || "",
                              unitPrice: "",
                              promotion2x1: false,
                            }));
                          }}
                      ...
                      <label className="field">
                        <span>Descuento o campaña *</span>
                        <select
                          value={saleDraft.discountCode}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                        ...
                        >
                          <option value="">Selecciona conscientemente</option>
                        ...
                        <small>
                          Usa las mismas reglas oficiales del abono del
                          contacto.
                        </small>
                      </label>
                      <label className="field">
                      ...
                      <label className="field">
                        <span>Precio con descuento por unidad *</span>
                        <input
                          type="number"
                          min="0"
                          step="0.01"
                        ...
                        />
                        <small>
                          {saleQuoteState === "loading"
                            ? "Calculando con el catálogo…"
                            : saleQuoteState === "error"
                              ? saleQuoteError
                              : saleQuote
                                ? `${saleQuote.discountName} · Lista ${currency.format(saleQuote.commercialValue)} · Descuento ${currency.format(saleQuote.discountAmount)} · Total ${currency.format(draftDocumentedTotal)}${saleQuote.pricingMode === "two_for_one" ? ` · ${saleQuote.chargedUnits} con cargo + ${saleQuote.bonusUnits} bonificados` : ""}`
                                : "Selecciona zona, descuento y cantidad."}
                        </small>
                      </label>
                      <label className="field">
                        <span>Lugares de estacionamiento</span>
                  ...
                  {!editingSale && (
                    <>
                      <label className="field">
                        ...
                        />
                        <small>
                          Puede ser $0, apartado o liquidación. Saldo:{" "}
                          {currency.format(
                            Math.max(
                              0,
                              draftDocumentedTotal -
                                Number(saleDraft.paymentAmount || 0),
                            ),
                          )}
                        </small>
                      </label>
                      ...
                      <label className="field">
                        <span>Referencia del cobro</span>
                        <input
                          value={saleDraft.paymentReference}
                          onChange={(event) =>
                            setSaleDraft((current) => ({
                              ...current,
                  ...
                  {editingSale && (
                    <label className="field field--full">
                      ...
                      <textarea
                        ...
                        maxLength="500"
                        rows="3"
                        value={saleDraft.correctionReason}
                        onChange={(event) =>
                          setSaleDraft((current) => ({
                            ...current,
                        ...
                        placeholder="Describe qué dato estaba incorrecto y por qué se corrige."
                      />
                      <small>
                        Quedará registrado en la bitácora de auditoría. Los
                        cobros existentes no se modifican.
                      </small>
                    </label>
                  )}
                </div>
              ...
              <footer className="drawer-footer">
                ...
                >
                  {savingSale
                    ...
                    : editingSale
                      ? "Guardar corrección"
                      : saleDraft.closeStage === "won"
                        ? "Registrar venta ganada"
                        : "Registrar apartado"}
                </button>
              </footer>
            </form>
...
function MorePage({ page, isDemo, campaigns, configurationFixtures }) {
  const config = {
    campaigns: {
      ...
      title: "Campañas y envíos",
      description:
        "Audiencias, consentimiento y resultados separados del contacto humano.",
      icon: "send",
    },
    rewards: {
      eyebrow: "Fidelidad",
      title: "Recompensas",
      description:
        "Hitos, beneficios y canjes de abonados en una vista controlada.",
      icon: "star",
    },
    catalogs: {
      ...
      title: "Catálogos",
      description:
        "Estatus, canales, zonas, productos y precios versionados por temporada.",
      icon: "layers",
    },
  }[page];
  return (
    <div className="page-wrap">
      ...
      {page === "campaigns" ? (
        ...
        <ConfigurationPanel
          content={configurationFixtures[page] || []}
          config={config}
          isDemo={isDemo}
        />
      )}
  ...
  return (
    <section className="panel list-panel">
      <div className="notice notice--important">
        ...
        <div>
          <strong>Envíos todavía no habilitados</strong>
          <p>
            El módulo no habilita envíos hasta conectar un API que valide el
            consentimiento de cada destinatario.
          </p>
        </div>
      </div>
      {campaigns.length ? (
            ...
            <tbody>
              {campaigns.map((campaign) => (
                <tr key={campaign.name}>
                  ...
                  <td>{campaign.sent.toLocaleString("es-MX")}</td>
                  <td>{campaign.delivered.toLocaleString("es-MX")}</td>
                  <td>{campaign.responses}</td>
                  <td>
                    {campaign.delivered
                      ? (
                          (campaign.responses / campaign.delivered) *
                          100
                        ).toFixed(1)
                      : "0.0"}
                    %
        ...
        <EmptyState
          title="Sin campañas conectadas"
          body="El módulo está preparado, pero el API aún no publica un listado de campañas."
        />
      )}
    </section>
  );
    ...
    <div className="configuration-grid">
      <section className="panel config-list">
        <div className="panel-heading">
          <div>
            <span className="panel-kicker">
              {isDemo ? "VISTA PREVIA" : "FASE POSTERIOR"}
            </span>
            <h2>
              {isDemo
                ? "Elementos ilustrativos"
                : "Sin función operativa en esta entrega"}
            </h2>
          </div>
        </div>
        {content.map(([name, detail, status]) => (
          <button
            type="button"
            disabled
            key={name}
            aria-label={`${name}: función no disponible en esta entrega`}
          >
        ...
        {!content.length && (
          <EmptyState
            ...
            body="Esta primera entrega no consulta ni modifica datos para esta sección."
          />
        )}
      </section>
      <aside className="panel config-help">
        <span className="summary-icon summary-icon--blue">
          <Icon name={config.icon} />
        </span>
        <h2>Alcance futuro</h2>
        <p>
          Esta es únicamente una propuesta visual. Antes de activarla se deberán
          definir sus reglas, endpoints, permisos y eventos de auditoría.
        </p>
        <ul>
          <li>Sin acciones activas en esta versión</li>
          <li>Sin almacenamiento local en el navegador</li>
          <li>Implementación sujeta a aprobación</li>
        </ul>
      </aside>
    </div>
  );
...
function ContactDrawer({
  ...
  const memberships = (drawer.memberships || []).filter(
    (item) => item.seasonCode === "LMP-2026-27",
  );
  const initialMembership =
    drawer.membership || currentSeasonMembership(memberships);
  const [selectedMembershipId, setSelectedMembershipId] = useState(
    initialMembership?.id || "new",
  );
  const membership =
    selectedMembershipId === "new"
      ? null
  ...
  const [taskDraft, setTaskDraft] = useState({
    assignedTo: existing.executiveId || user.id,
    dueAt: localDateTimeInput(new Date(Date.now() + 24 * 60 * 60 * 1000)),
    priority: "normal",
    description: "",
  });
  const nameParts = String(existing.name || "")
    .trim()
    .split(/\s+/);
  const initialForm = {
    id: existing.id,
    ...
    firstName:
      ...
      "",
    lastName:
      existing.lastName || (nameParts.length > 1 ? nameParts.at(-1) : ""),
    email: existing.email || "",
    phone: existing.phone || "",
    type:
      existing.type ||
      (drawer.kind === "prospect" ? "Prospecto" : "Por renovar"),
    ...
    commercialSegment: existing.commercialSegment || "",
    suiteNumber: existing.suiteNumber || "",
  };
  ...
  const mayEdit =
    !deleted &&
    (editing ? canEditContacts(user, existing) : canCreateContacts(user));
  ...
  const mayChangeSubscriberStatus = hasPermission(
    user,
    PERMISSIONS.MEMBERSHIP_WRITE,
  );
  const mayChangeConsent = hasPermission(user, PERMISSIONS.CONTACT_WRITE_ALL);
  const mayManageMembership =
    editing && !deleted && hasPermission(user, PERMISSIONS.MEMBERSHIP_WRITE);
  const mayLogInteraction =
    editing &&
    !deleted &&
    canEditContacts(user, existing) &&
    hasPermission(user, PERMISSIONS.INTERACTION_WRITE);
  const mayCreateTask =
    editing &&
    !deleted &&
    canEditContacts(user, existing) &&
    (hasPermission(user, PERMISSIONS.TASK_WRITE_ALL) ||
      hasPermission(user, PERMISSIONS.TASK_WRITE_ASSIGNED));
  ...
  useEffect(() => {
    const available = memberships.some(
  ...
  async function submit(event) {
    ...
    if (!form.email.trim() && !form.phone.trim()) {
      setContactMethodError(
        "Captura al menos un correo o un teléfono para continuar.",
      );
      return;
    }
    if (form.commercialSegment === "Compromisos" && !form.suiteNumber.trim()) {
      setContactMethodError(
        "Captura el número de Suite para guardar el segmento Compromisos.",
      );
      return;
    }
    setContactMethodError("");
    ...
    if (
      ...
      ["Apartado", "Ganado"].includes(form.stage)
    ) {
      const remainingChanges = changedFields.filter(
        (field) => field !== "stage",
      );
      if (remainingChanges.length) {
        setContactMethodError(
          "Guarda primero los demás cambios; después selecciona Apartado o Ganado para completar la venta.",
        );
        return;
      }
  ...
  async function registerInteraction() {
    ...
    setActionError("");
    try {
      await onCreateInteraction(existing, {
        ...interactionDraft,
        outcome: interactionDraft.outcome.trim(),
      ...
      setActionError(
        "No fue posible registrar la interacción. Revisa los datos e intenta nuevamente.",
      );
    } finally {
      setActionSaving("");
    }
  ...
  async function scheduleTask() {
    ...
    setActionError("");
    try {
      await onCreateTask(existing, {
        ...taskDraft,
        assignedTo: mayAssignTask ? taskDraft.assignedTo : user.id,
      ...
      setActionError(
        "No fue posible programar la tarea. Revisa los datos e intenta nuevamente.",
      );
    } finally {
      setActionSaving("");
    }
  ...
  return (
    ...
    >
      <aside
        ...
        aria-busy={savingContact || membershipSaving || Boolean(actionSaving)}
      >
        <header className="drawer-header">
          <div>
            <span className="eyebrow">
              {editing ? `ID ${existing.id}` : "Nuevo registro"}
            </span>
            <h2 id="drawer-title" ref={headingRef} tabIndex="-1">
              {editing
                ? existing.name
                : drawer.kind === "prospect"
                  ? "Crear prospecto"
                  : "Crear contacto"}
            </h2>
          </div>
          <button
            ...
            disabled={
              savingContact || membershipSaving || Boolean(actionSaving)
            }
            onClick={onClose}
          >
        ...
        <form onSubmit={submit}>
          <div className="drawer-body">
            {editing && (
              <div className="contact-summary">
                <span className="large-avatar">
                  {existing.initials || initials(existing.name)}
                </span>
                <div>
                  <StatusPill>{existing.type}</StatusPill>
                  <span>
                    <Icon name="clock" size={14} />
                    Última gestión: {existing.lastContact}
                  </span>
                </div>
              </div>
            )}
            {deleted && (
              <div className="notice notice--important">
                ...
                <div>
                  ...
                  <p>
                    Está visible únicamente para revisión y restauración
                    autorizada.
                  </p>
                </div>
              </div>
            ...
            {!mayEdit && !deleted && (
              <div className="notice">
                ...
                <div>
                  ...
                  <p>
                    Solo puedes editar contactos asignados a tu usuario o
                    aquellos habilitados por tus permisos.
                  </p>
                </div>
              </div>
            )}
            <fieldset disabled={!mayEdit}>
              ...
              <div className="form-grid">
                ...
                <label className="field">
                  <span>Correo corporativo o personal</span>
                  <input
                    type="email"
                    value={form.email}
                ...
                <p
                  ...
                  className={
                    contactMethodError
                      ? "form-error field--full"
                      : "field-help field--full"
                  }
                  role={contactMethodError ? "alert" : undefined}
                >
                  {contactMethodError || "Captura al menos correo o teléfono."}
                </p>
                <label className="field field--full">
                  <span>Municipio</span>
                  <input
            ...
            <fieldset disabled={!mayEdit}>
              ...
              <div className="form-grid">
                <label className="field">
                  <span>Estatus de abonado</span>
                  <select
                    disabled={editing && !mayChangeSubscriberStatus}
                    value={form.type}
                  ...
                  {editing && !mayChangeSubscriberStatus && (
                    <small>Se actualiza desde membresías.</small>
                  )}
                </label>
                <label className="field">
                ...
                <label className="field">
                  <span>Zona del abono</span>
                  <select
                    value={form.commercialSegment}
                    onChange={(event) => {
                      update("commercialSegment", event.target.value);
                      if (event.target.value !== "Compromisos")
                        update("suiteNumber", "");
                    }}
                  >
                    <option value="">Sin definir</option>
                  ...
                  <small>
                    La orden asociada continúa siendo la fuente comercial
                    principal.
                  </small>
                </label>
                {form.commercialSegment === "Compromisos" && (
                  <label className="field">
                    <span>Número de Suite *</span>
                    <input
                      required
                      ...
                      onChange={(event) =>
                        update("suiteNumber", event.target.value)
                      }
                      placeholder="Ej. 14"
                    />
                  </label>
                ...
                {editing && (
                  <>
                    <div className="field">
                      <span>Temporadas verificadas</span>
                      <output className="derived-value">
                        {form.seasons || "No consta"}
                      </output>
                      <small>Calculadas desde membresías.</small>
                    </div>
                    <div className="field">
                      <span>Temporadas declaradas</span>
                      <output className="derived-value">
                        {form.declaredSeasons ?? "No consta"}
                      </output>
                      <small>Dato informado durante el alta.</small>
                    </div>
                    <div className="field">
                      <span>Cantidad de abonos gestionados</span>
                      <output className="derived-value">{form.seats}</output>
                      <small>Incluye abonos activos y por renovar.</small>
                    </div>
                    <div className="field">
                      <span>Origen comercial</span>
                      <output className="derived-value">
                        {form.businessSourceLabel}
                      </output>
                      <small>Capturado en el alta manual.</small>
                    </div>
                  </>
                )}
                <label className="field">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      update("preferredChannel", event.target.value)
                    }
                  >
                    <option value="">Sin definir</option>
                    <option value="phone">Llamada</option>
                ...
                <label className="field">
                  ...
                  {mayAssign ? (
                    <select
                      ...
                      onChange={(event) =>
                        update("executiveId", event.target.value)
                      }
                    >
                      <option value="">Sin asignar</option>
                      {form.executiveId &&
                        !executiveOptions.some(
                          (item) => item.id === form.executiveId,
                        ) && (
                          <option value={form.executiveId}>
                            {form.executive}
                          </option>
                        )}
                      {executiveOptions.map((item) => (
                    ...
                    <output className="derived-value">
                      {editing ? form.executive : user.name}
                    </output>
                  )}{" "}
                  {!mayAssign && (
                    <small>
                      {editing
                        ? "Requiere permiso de asignación."
                        : "El alta se asignará automáticamente a tu cartera."}
                    </small>
                  )}
                </label>
                <label className="field field--full">
                  <span>Consentimiento de contacto</span>
                  <select
                    disabled={editing && !mayChangeConsent}
                    value={form.consent}
                  ...
                  <small>
                    {editing && !mayChangeConsent
                      ? "Solo Supervisor o Administrador puede modificar este dato."
                      : "La fecha y la fuente del cambio se registran en el servidor."}
                  </small>
                </label>
              </div>
            </fieldset>
              ...
              >
                <div className="membership-orders-heading">
                  <div>
                    ...
                    <h3 id="membership-orders-title">Abonos del contacto</h3>
                  </div>
                  {mayManageMembership && (
                    <button
                      type="button"
                      ...
                      onClick={() => onRequestSaleClosure(existing, "Apartado")}
                    >
                ...
                {memberships.length > 0 && (
                  <label className="field">
                    ...
                    >
                      {selectedMembershipId === "new" && (
                        <option value="new">Nueva orden</option>
                      )}
                      {memberships.map((item, index) => (
                        <option key={item.id} value={item.id}>
                          Orden {memberships.length - index} · {item.seatCount}{" "}
                          {item.seatCount === 1 ? "abono" : "abonos"} ·{" "}
                          {item.localityName ||
                            item.membershipSection ||
                            "Sin localidad"}
                        </option>
                      ))}
                    </select>
                ...
                {membership ? (
                  <MembershipEditor
                    ...
                    onQuote={onQuoteMembershipPricing}
                    canEdit={mayManageMembership}
                    focusOnMount={drawer.focusMembership}
                    onSave={async (draft) => {
                      const saved = await onSaveMembership(
                        existing,
                        membership,
                        draft,
                      );
                      setSelectedMembershipId(saved?.id || membership.id);
                    }}
                    onSavingChange={setMembershipSaving}
                  />
                ) : (
                  <div className="manual-inline-note">
                    <strong>Sin detalle auxiliar</strong>
                    <span>
                      Registra o completa la orden desde Ventas; los indicadores
                      no se alimentan desde este apartado.
                    </span>
                  </div>
                )}
              </section>
            ...
            <fieldset disabled={!mayEdit}>
              ...
              <label className="field">
                ...
                <textarea
                  ...
                  placeholder="Anota contexto útil para la siguiente gestión."
                />
                <small>{form.note.length}/500 caracteres</small>
              </label>
            </fieldset>
            {(mayLogInteraction || mayCreateTask) && (
              ...
              >
                <div className="contact-actions-heading">
                  ...
                  <h3 id="contact-actions-title">Registrar seguimiento</h3>
                </div>
                {mayLogInteraction && (
                  <details>
                    <summary>
                    ...
                    <div className="action-form-grid">
                      ...
                      <label className="field field--full">
                        ...
                        <textarea
                          ...
                          placeholder="Detalle verificable de la gestión."
                        />
                      </label>
                      <label className="check-field field--full">
                        <input
                          type="checkbox"
                          checked={interactionDraft.isHumanContact}
                          onChange={(event) =>
                            setInteractionDraft((current) => ({
                              ...current,
                              isHumanContact: event.target.checked,
                            }))
                          }
                        />
                        <span>Esta gestión cuenta como contacto humano</span>
                      </label>
                      <div className="action-submit field--full">
                        <SecondaryButton
                          type="button"
                          icon="check"
                    ...
                    <div className="action-form-grid">
                      <label className="field">
                        ...
                        {mayAssignTask ? (
                          ...
                          <output className="derived-value">{user.name}</output>
                        )}
                      </label>
                      <label className="field">
            ...
            {confirmDelete && (
              <div className="delete-confirm" role="alert">
                ...
                <label className="field delete-reason">
          ...
          <footer className="drawer-footer">
            {editing && mayDelete && !confirmDelete ? (
              <button
                type="button"
            ...
            <div>
              <SecondaryButton
                type="button"
                disabled={savingContact || membershipSaving}
                onClick={onClose}
              >
                Cancelar
              </SecondaryButton>
