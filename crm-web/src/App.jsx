...
export function buildCommitmentItems({ suiteNumber, quantity, totalAmount, coverageSeasons = 2 }) {
  const seats = Number(quantity)
  const totalCents = Math.round(Number(totalAmount) * 100)
  if (!Number.isInteger(seats) || seats < 1 || !Number.isInteger(totalCents) || totalCents < 0) return []
  ...
  return [
    ...(higherPriceSeats ? [{ product, zone, quantity: higherPriceSeats, unitPrice: (baseCents + 1) / 100 }] : []),
    ...(seats - higherPriceSeats ? [{ product, zone, quantity: seats - higherPriceSeats, unitPrice: baseCents / 100 }] : []),
  ]
}
...
function initialSaleDraft() {
  return { externalOrderNumber: '', contactId: '', executiveId: '', commercialCategory: 'subscription', coverageSeasons: 1, suiteNumber: '', commitmentTotal: '', kind: 'new', closeStage: 'reserved', localityCode: '', discountCode: '', zone: '', promotion2x1: false, quantity: 1, unitPrice: '', soldAt: new Date().toISOString().slice(0, 10), paymentAmount: '', paymentMethod: 'Transferencia', paymentReference: '', notes: '', correctionReason: '', additionalHolders: [], seatDetails: [] }
}
...
function SalesPage({ sales, contacts, isDemo, user, availableExecutives, pricingCatalog, onQuoteMembershipPricing, onAddPayment, onCreateSale, onCorrectSale, onCancelSale, onCreate, saleClosure, onClearSaleClosure }) {
  ...
  const [segment, setSegment] = useState('Todos los segmentos')
  const [from, setFrom] = useState('')
  const [to, setTo] = useState('')
  ...
  const filteredSales = sales.filter((sale) => {
    ...
    return (!query || `${sale.externalOrderNumber || ''} ${sale.id} ${sale.contact} ${sale.zone}`.toLowerCase().includes(query))
      ...
      && (segment === 'Todos los segmentos' || sale.segment === segment)
  ...
  const draftDocumentedTotal = isCommitmentDraft ? Number(saleDraft.commitmentTotal || 0) : saleQuote?.netAmount ?? (draftChargedUnits * draftUnitPrice)
  useEffect(() => {
    saleQuoteRequest.current += 1
    const requestId = saleQuoteRequest.current
    setSaleQuote(null)
    setSaleQuoteError('')
    if (isCommitmentDraft || !saleDraft.localityCode || !saleDraft.discountCode || !Number.isInteger(draftQuantity) || draftQuantity < 1 || !onQuoteMembershipPricing) {
    ...
    return () => window.clearTimeout(timeout)
  }, [isCommitmentDraft, saleDraft.localityCode, saleDraft.discountCode, draftQuantity, onQuoteMembershipPricing])
  ...
  function openSaleCorrection(sale) {
    ...
    setSaleDraft({
      ...
      commercialCategory: sale.commercialCategory || (sale.segment === 'Compromisos' ? 'commitment' : 'subscription'),
      coverageSeasons: Number(sale.coverageSeasons || (sale.segment === 'Compromisos' ? 2 : 1)),
      suiteNumber: sale.suiteNumber || '',
      commitmentTotal: sale.segment === 'Compromisos' ? String(sale.total || '') : '',
  ...
  async function submitSale(event) {
    ...
    const items = isCommitmentDraft
      ? buildCommitmentItems({ suiteNumber: saleDraft.suiteNumber, quantity, totalAmount: saleDraft.commitmentTotal, coverageSeasons: saleDraft.coverageSeasons })
      : buildSaleItems({ kind: saleDraft.kind, zone: saleDraft.zone, quantity, unitPrice, promotion2x1, discountCode: saleQuote?.discountCode, discountName: saleQuote?.discountName, pricingMode: saleQuote?.pricingMode, chargedUnits: saleQuote?.chargedUnits, bonusUnits: saleQuote?.bonusUnits })
    ...
    const invalidCommercialTerms = isCommitmentDraft
      ? !saleDraft.suiteNumber.trim() || Number(saleDraft.coverageSeasons) !== 2 || !Number.isFinite(Number(saleDraft.commitmentTotal)) || Number(saleDraft.commitmentTotal) < 0 || !items.length
      : !saleDraft.localityCode || !saleDraft.discountCode || saleQuoteState !== 'ready' || !saleQuote || !Number.isFinite(unitPrice) || unitPrice < 0
    if (!saleDraft.externalOrderNumber.trim() || !saleDraft.contactId || !saleDraft.executiveId || !Number.isInteger(quantity) || quantity < 1 || invalidCommercialTerms || invalidHolderDistribution || (!editingSale && (paymentAmount < 0 || paymentAmount > totalAmount))) {
      setSaleError(isCommitmentDraft ? 'Completa número de orden, titular, ejecutivo, número de suite, butacas y valor anual.' : 'Completa número de orden, titular, ejecutivo, zona, descuento y cantidad; espera a que termine la cotización antes de guardar.')
      return
    }
    ...
    try {
      ...
      const payload = { externalOrderNumber: saleDraft.externalOrderNumber.trim(), commercialCategory: saleDraft.commercialCategory, coverageSeasons: Number(saleDraft.coverageSeasons), suiteNumber: isCommitmentDraft ? saleDraft.suiteNumber.trim() : undefined, saleType: saleDraft.kind, closeStage: saleDraft.closeStage, contactId: saleDraft.contactId, executiveId: saleDraft.executiveId, seasonCode: 'LMP-2026-27', status: saleDraft.closeStage === 'won' ? 'confirmed' : 'reserved', soldAt: new Date(`${saleDraft.soldAt}T12:00:00`).toISOString(), currency: 'MXN', notes: [promotion2x1 ? 'Promoción 2x1 aplicada automáticamente desde el catálogo oficial.' : '', saleDraft.notes].filter(Boolean).join(' ') || undefined, items, pricing: isCommitmentDraft ? null : { localityCode: saleDraft.localityCode, discountCode: saleDraft.discountCode, seatCount: quantity }, holderAssignments, ...(hasSeatDetails ? { seatDetails } : {}) }
  ...
  return (
    <div className="page-wrap">
      ...
      {saleOpen && <div className="drawer-backdrop" role="presentation"><aside className="drawer drawer--manual" role="dialog" aria-modal="true" aria-labelledby="sale-title"><header className="drawer-header"><div><span className="eyebrow">{editingSale ? 'Corrección auditada' : 'Alta guiada'}</span><h2 id="sale-title">{editingSale ? 'Corregir venta' : 'Nueva venta'}</h2><p>{editingSale ? 'Modifica los datos comerciales. La venta original y sus cobros se conservarán en el historial.' : 'La orden, sus abonos y el cobro inicial quedarán vinculados al titular.'}</p></div><button type="button" className="icon-button" aria-label="Cerrar" onClick={closeSaleDrawer}>×</button></header><form onSubmit={submitSale}><div className="drawer-body"><div className="form-grid">
        ...
        <label className="field"><span>Clasificación *</span><select value={saleDraft.commercialCategory} onChange={(event) => setSaleDraft((current) => ({ ...current, commercialCategory: event.target.value, coverageSeasons: event.target.value === 'commitment' ? 2 : 1, suiteNumber: event.target.value === 'commitment' ? current.suiteNumber : '', commitmentTotal: event.target.value === 'commitment' ? current.commitmentTotal : '' }))}><option value="subscription">Abono de temporada</option><option value="commitment">Compromiso anual de suite</option></select></label>
        {!isCommitmentDraft && <label className="field"><span>Tipo *</span><select value={saleDraft.kind} onChange={(event) => setSaleDraft((current) => ({ ...current, kind: event.target.value }))}><option value="new">Abono nuevo</option><option value="renewal">Renovación</option></select></label>}
        ...
        {isCommitmentDraft ? <>
          <label className="field"><span>Número de suite *</span><input required maxLength="40" value={saleDraft.suiteNumber} onChange={(event) => setSaleDraft((current) => ({ ...current, suiteNumber: event.target.value }))} placeholder="Ej. 14"/><small>Se guarda como identificador de la suite dentro de la orden.</small></label>
          <label className="field"><span>Temporalidad *</span><select value={saleDraft.coverageSeasons} onChange={(event) => setSaleDraft((current) => ({ ...current, coverageSeasons: Number(event.target.value) }))}><option value="2">Anual · 2 temporadas</option></select><small>Comprende la temporada LMP y la temporada complementaria del año.</small></label>
          <label className="field"><span>Número de butacas *</span><input type="number" min="1" max="100" value={saleDraft.quantity} onChange={(event) => setSaleDraft((current) => ({ ...current, quantity: event.target.value }))}/></label>
          <label className="field"><span>Valor anual comprometido *</span><input type="number" min="0" step="0.01" value={saleDraft.commitmentTotal} onChange={(event) => setSaleDraft((current) => ({ ...current, commitmentTotal: event.target.value }))}/><small>Importe completo de la suite por las dos temporadas. Se suma a venta documentada; el cobro inicial se registra por separado.</small></label>
        </> : <>
        ...
        <details className="field field--full seat-secondary-details"><summary>{isCommitmentDraft ? 'Identificadores de las butacas de la suite' : 'Datos secundarios por butaca'}</summary><p>{isCommitmentDraft ? 'Opcional. Registra una referencia por lugar si ya cuentas con ella.' : 'Opcional. Puedes guardar la orden sin completar butaca, talla o personalización.'}</p><div className="seat-secondary-grid">{Array.from({ length: Math.max(1, Number(saleDraft.quantity || 1)) }, (_, index) => { const detail = saleDraft.seatDetails[index] || {}; return <fieldset key={`sale-seat-${index + 1}`}><legend>{isCommitmentDraft ? 'Butaca' : 'Abono'} {index + 1}</legend><label className="field"><span>Butaca</span><input maxLength="100" value={detail.seatIdentifier || ''} onChange={(event) => updateSaleSeatDetail(index, 'seatIdentifier', event.target.value)} placeholder="Ej. Lugar 1"/></label>{!isCommitmentDraft && <><label className="field"><span>Talla de jersey</span><select value={detail.jerseySize || ''} onChange={(event) => updateSaleSeatDetail(index, 'jerseySize', event.target.value)}><option value="">Sin definir</option><option>S</option><option>M</option><option>L</option><option>XL</option><option>2XL</option></select></label><label className="field field--full"><span>Personalización-butaca</span><input maxLength="120" value={detail.personalization || ''} onChange={(event) => updateSaleSeatDetail(index, 'personalization', event.target.value)} placeholder="Opcional"/></label></>}</fieldset> })}</div></details>
