...
describe('CRM web en modo demostración', () => {
  it('separa estacionamientos de los abonos en todas las combinaciones del filtro', () => {
    ...
    expect(dashboardSegmentsForFilters(segments, [])).toEqual({ Compromisos: 2, VIP: 3, Preferente: 5, General: 7, Estacionamientos: 11 })
  ...
  it('registra una interacción desde el contacto con una acción real', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Contacto', pressed: true })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Orden' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Personalización' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'In-season' })).toBeInTheDocument()
