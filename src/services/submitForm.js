const PENDING_QUEUE_KEY = 'charros-survey-pending'
...
function readPendingQueue() {
  ...
  try {
    ...
    console.warn('Unable to read pending queue', error)
    return []
  }
}
...
async function flushPendingQueue(endpoint) {
  ...
  for (const item of queue) {
    try {
      ...
      remaining.push(item)
      console.warn('Unable to flush queued submission', error)
    }
  }
...
export async function submitForm(payload){
  ...
  if (endpoint) {
    try {
      ...
      if (localQueueEnabled && typeof window !== 'undefined') {
        ...
        console.warn('Remote submission failed. Stored locally for retry.', error)
  ...
  if (localQueueEnabled && typeof window !== 'undefined') {
    const pendingCount = queueSubmission(payload)
    console.warn('No submission endpoint configured. Stored locally for later sync.', pendingCount)
    return { ok: true, queuedLocally: true, pendingCount }
  }

  // Mock submit for local development without any endpoint
  console.warn('No submission endpoint configured. Using mock submit.')
